...
- [x] 22. ごはん日記 — ドメインロジックの実装
  - [x] 22.1 日記分析関数を実装する
    - `src/domain/diary.ts` に `buildDateRecipeMap()` 関数を作成（保存済み献立から日付→レシピのマッピングを構築）
    - `computeRecipeRanking()` 関数を作成（レシピ使用回数ランキング、期間指定対応）
    - `computeCategoryBalance()` 関数を作成（カテゴリ別使用比率、期間指定対応）
    - `computeStaleRecipes()` 関数を作成（最終使用日が古い順、未使用レシピは先頭）
    - _要件: 11.5, 12.1, 12.2, 13.1, 13.2, 14.1, 14.2, 14.3_

- [x] 23. ごはん日記 — カスタムフックの実装
  - [x] 23.1 useDiary フックを実装する
    - `src/hooks/useDiary.ts` を作成
    - 月選択状態（selectedMonth）と期間指定状態（dateRange）を管理
    - ドメイン関数を呼び出して各分析データを算出
    - 保存済み献立とレシピデータを受け取り、メモ化された分析結果を返す
    - _要件: 11.1, 11.3, 12.4, 13.3_

- [x] 24. ごはん日記 — UIコンポーネントの実装
  - [x] 24.1 カレンダー表示コンポーネントを作成する
    - `src/components/diary/CalendarView.tsx` — 月カレンダー表示（前月/翌月切り替え）
    - `src/components/diary/DayDetail.tsx` — 日付タップ時の献立詳細表示
    - カレンダーの各日付セルにレシピ名を表示
    - _要件: 11.1, 11.2, 11.3, 11.4, 11.5_

  - [x] 24.2 レシピランキングコンポーネントを作成する
    - `src/components/diary/RecipeRanking.tsx` — 使用回数順のレシピ一覧（上位10件）
    - 期間指定フィルター付き
    - _要件: 12.1, 12.2, 12.3, 12.4_

  - [x] 24.3 カテゴリバランスコンポーネントを作成する
    - `src/components/diary/CategoryBalance.tsx` — カテゴリ別使用比率の横棒グラフ表示
    - CSSのみでグラフを実現（外部ライブラリ不使用）
    - 期間指定フィルター付き
    - _要件: 13.1, 13.2, 13.3, 13.4_

  - [x] 24.4 最近作ってないレシピコンポーネントを作成する
    - `src/components/diary/StaleRecipes.tsx` — 最終使用日が古い順のレシピ一覧
    - 未使用レシピは「未使用」ラベル付きで先頭に表示
    - タップでレシピ詳細を表示
    - _要件: 14.1, 14.2, 14.3, 14.4_

  - [x] 24.5 ごはん日記メインページを作成する
    - `src/components/diary/DiaryPage.tsx` — サブタブ切り替え（カレンダー/ランキング/バランス/最近作ってない）
    - `src/components/diary/index.ts` — バレルエクスポート
    - _要件: 11.1_

- [x] 25. ごはん日記 — アプリ統合
  - [x] 25.1 ナビゲーションとApp.tsxにごはん日記を統合する
    - `NAV_ITEMS` に `{ id: 'diary', label: 'ごはん日記', icon: '📔' }` を追加
    - `App.tsx` に diary タブのルーティングを追加
    - useDiary フックを App.tsx で使用し、DiaryPage にデータを渡す
    - _要件: 11.1_

- [x] 26. チェックポイント — ごはん日記の最終確認
  - すべてのテストが通ることを確認し、不明点があればユーザーに質問する。
